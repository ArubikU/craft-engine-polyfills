...
RP = os.path.join(ROOT, "demo", "resourcepack", "assets", "minecraft")
TEX = os.path.join(RP, "textures")
MODELS = os.path.join(RP, "models")
BAKE_DIR = os.path.join(TEX, "font", "image", "baked")
ITEMS_DIR = os.path.join(ROOT, "demo", "configuration", "items")
IMAGES_YML = os.path.join(ROOT, "demo", "configuration", "images.yml")
...
ICONS = {
    ...
    "motor":    {"image": "i_motor", "kind": "model", "model": "block/custom/vapor_motor/vapor_motor"},
...
BLUEPRINTS = {
    ...
    "depot_blueprint": {
        ...
        "craft": {"pattern": ["PPP", "PGP", "PPP"],
                  "ingredients": {"P": "minecraft:paper", "G": "demo:iron_gear"}},
    },
...
def write_images():
    ...
    for key in sorted(inp | outp):
        ...
        if ic["kind"] == "model":
            ...
            file_ref = ic["tex"]
        if key in inp:
            out += ["  demo:%s:" % ic["image"], "    height: %d" % ICON_HEIGHT,
                    "    ascent: %d" % ICON_ASCENT, "    font: minecraft:default",
                    "    file: %s" % file_ref, "    grid_size: 1,1"]
        if key in outp:
            out += ["  demo:%s_o:" % ic["image"], "    height: %d" % OUTPUT_HEIGHT,
...
def tag_in(key):
    return "<image:demo:%s:0:0>" % ICONS[key]["image"]


def tag_out(key):
    return "<image:demo:%s_o:0:0>" % ICONS[key]["image"]
...
NAME_COLORS = {"conveyor_blueprint": "aqua", "depot_blueprint": "gold",
               ...
               "logistics_blueprint": "light_purple"}
...
def write_blueprint(bp_id, bp):
    ...
    if len(recs) > 1:
        L.append('        - "<!i><light_purple><lang:demo.bp.multi>"')
    L += ['        - "<!i><gray><lang:demo.bp.craftable_on>"',
          '        - "<!i>  <blue><lang:demo.bp.workbench>"',
          '        - "<!i><gray><lang:demo.bp.usage>"',
          '        - "<!i>  <blue><lang:demo.bp.placeable>"',
          '        - "<!i> "',
          '        - "<!i><dark_gray><lang:demo.bp.reusable>"',
          '        - "<!i><dark_gray><lang:demo.bp.autofill>"']

    name = '<!i><%s><lang:item.demo.%s>' % (NAME_COLORS.get(bp_id, "white"), bp_id)
    cr = bp["craft"]
    doc = ["# AUTO-GENERATED by tools/blueprint_gen.py - do not edit by hand.",
           "items:", "  demo:%s:" % bp_id, "    data:",
           '      item_name: "%s"' % name]
    doc += L
    doc += ["    model: %s" % bp["model"], "",
            "recipes:", "  demo:%s:" % bp_id, "    type: shaped", "    pattern:"]
    doc += ['      - "%s"' % r for r in cr["pattern"]]
    doc.append("    ingredients:")
    doc += ['      %s: %s' % (k, v) for k, v in cr["ingredients"].items()]
    doc += ["    result:", "      id: demo:%s" % bp_id, "      count: 1"]
