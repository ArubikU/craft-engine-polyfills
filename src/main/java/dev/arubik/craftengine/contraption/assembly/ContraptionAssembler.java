...
public final class ContraptionAssembler {
     ...
     * ROTATIONAL rpm. Prefer {@link #assemble(World, BlockPos, Key, double)} in
     ...
     * ROTATIONAL type (see {@link #attachDefaultBehavior}); {@code suPerBlock}
     ...
     * to {@link #assemble(World, BlockPos, Key, double, double)} but (a) records
    ...
    public static ContraptionEntity assemble(World bukkitWorld, BlockPos bearing, Key type,
        ...
        if (Key.of("polyfills", "linear").equals(type)) {
        ...
        ContraptionState state = new ContraptionState(UUID.randomUUID(), level.dimension(), captured.level(),
     ...
     * bearing uses ({@link #assemble(World, BlockPos, Key, double, double)}), but with
     * PHYS type so {@link #attachDefaultBehavior} attaches a
     ...
     *   block's PHYS type via {@code BearingBlockBehavior#typeAt} and routes it through
    ...
    public static ContraptionEntity assemblePhysics(World bukkitWorld, BlockPos physAnchor) {
        ...
        double suPerBlock = dev.arubik.craftengine.contraption.behavior.BearingBlockBehavior.suPerBlockAt(level,
                physAnchor);
        return assemble(bukkitWorld, physAnchor, Key.of("polyfills", "phys"), rpm, suPerBlock, null);
    }
    ...
    public static ContraptionEntity assemblePiston(World bukkitWorld, BlockPos bearing) {
        Level level = ((CraftWorld) bukkitWorld).getHandle();
        net.minecraft.resources.ResourceKey<Level> worldKey = level.dimension();
        ...
        Set<BlockPos> structure = new HashSet<>(GlueRegistry.structureAt(worldKey, front));
        structure.addAll(GlueRegistry.structureAt(worldKey, bearing));
        ...
        if (fireAssembleCancelled(bukkitWorld, bearing, Key.of("polyfills", "linear"), structure, null)) {
            return null;
        }
        ContraptionCapture.Result captured = ContraptionCapture.capture(level, structure, bearing);
        ContraptionCapture.captureGlueEdges(worldKey, captured.level(), bearing);
        ...
        ContraptionState state = new ContraptionState(UUID.randomUUID(), worldKey, captured.level(),
                bearing.getX(), bearing.getY(), bearing.getZ());
        state.setFurniture(furnitureResult.furniture());
        for (MovementBehavior autoBehavior : captured.autoBehaviors()) {
            state.addBehavior(autoBehavior);
        ...
        attachDefaultBehavior(level, bearing, state, Key.of("polyfills", "linear"), DEFAULT_ROTATIONAL_RPM, suPerBlock);
        ContraptionEntity entity = ContraptionManager.register(new ContraptionEntity(state));
        dev.arubik.craftengine.contraption.persistence.BlockAnchoredContraptionStore.save(state, bearing,
                Key.of("polyfills", "linear"), DEFAULT_ROTATIONAL_RPM, suPerBlock);
        fireAssembled(entity);
        return entity;
    }
    ...
    public static ContraptionEntity assembleExplicitRetracting(World bukkitWorld, BlockPos bodyPos,
        ...
        ContraptionCapture.captureGlueEdges(level.dimension(), captured.level(), bodyPos);
        ContraptionFurnitureCapture.Result furnitureResult =
        ...
        dev.arubik.craftengine.contraption.persistence.BlockAnchoredContraptionStore.save(state, bodyPos,
                Key.of("polyfills", "linear"), speed, suPerBlock);
     ...
     */
    public static void attachDefaultBehavior(Level level, BlockPos bearing, ContraptionState state, Key type,
            double rotationalRpm, double suPerBlock) {
        ...
        } else if (Key.of("polyfills", "rotational").equals(type)) {
            state.addBehavior(new RotationalBearingBehavior(motorPos, rotationalRpm, suPerBlock));
        } else if (Key.of("polyfills", "phys").equals(type)) {
        ...
        } else if (Key.of("polyfills", "vehicle").equals(type)) {
        ...
        } else if (Key.of("polyfills", "ghast").equals(type)) {
     ...
     */
    public static void nudgeRidersUp(java.util.Set<UUID> riderIds) {
        if (riderIds.isEmpty()) {
            return;
        }
     ...
     */
    static void translateGlueToRestorePosition(net.minecraft.resources.ResourceKey<Level> worldKey, ContraptionState state, BlockPos snappedBearing,
            int quarterTurns) {
        if (state.level() == null) {
            return;
        }
     ...
     */
    public static boolean fireAssembleCancelled(World world, BlockPos bearing, Key type,
    ...
    public static void fireDisassembled(UUID contraptionId, World world, BlockPos snappedBearing,
