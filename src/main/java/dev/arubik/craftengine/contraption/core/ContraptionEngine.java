...
public final class ContraptionEngine {
    ...
    public static void tickAll() {
        ...
        // retracts them (2026-07-03 goal). See LinearContraptionType.
        try {
            dev.arubik.craftengine.contraption.type.LinearContraptionType.tickExtendedSolids();
        ...
        if (minecartDeadAtEnd != null) {
            for (ContraptionEntity entity : minecartDeadAtEnd) {
                ...
                try {
                    ...
                    dev.arubik.craftengine.contraption.listener.BearingHammerListener.forgetAssembled(entity.state().id());
                } catch (Throwable t) {
        ...
        if (disassembleAtEnd != null) {
            for (ContraptionEntity entity : disassembleAtEnd) {
                ...
                try {
                    ...
                    // retract them (LinearContraptionType). Only for a piston in a "become real
                    // blocks" mode.
    ...
    /** Snapshots a euler/robin_euler piston's dropped-load positions into {@link dev.arubik.craftengine.contraption.type.LinearContraptionType}. */
    private static void recordEulerDrop(ContraptionEntity entity) {
        ...
        // BearingBlockBehavior#createBlockEntityController). Removed again on re-grab (LinearContraptionType).
        ...
        dev.arubik.craftengine.contraption.type.LinearContraptionType.recordExtendedSolid(
                state.worldId(), state.originBearingBlockPos(), piston.direction(),
    ...
    /**
     * Resolves the Bukkit World from a ResourceKey<Level>.
     * Returns null under pure-JVM unit test (no live server) or if world not found.
     */
