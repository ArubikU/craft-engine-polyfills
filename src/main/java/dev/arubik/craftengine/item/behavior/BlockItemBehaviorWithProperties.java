...
public class BlockItemBehaviorWithProperties extends BlockBoundItemBehavior {
   public static class Factory implements ItemBehaviorFactory<ItemBehavior> {
      ...
      @Override
      public ItemBehavior create(Pack pack, Path path, String name, Key key, Map<String, Object> arguments) {
         Object id = arguments.get("block");
         if (id == null) {
            throw new LocalizedResourceConfigException("warning.config.item.behavior.block.missing_block",
                  new IllegalArgumentException("Missing required parameter 'block' for block_item behavior"),
                  new String[0]);
         } else {
      ...
      if (optionalBlock.isEmpty()) {
         ...
         if (context.getClickedFace() == Direction.UP && pos.y() >= maxY) {
            if (player != null) {
               ...
               cePlayer.platformPlayer()
                     .sendMessage(net.kyori.adventure.text.Component.translatable("build.tooHigh")
                           .args(net.kyori.adventure.text.Component.text(maxY))
            ...
            if (blockStateToPlace == null) {
               ...
               if (player != null) {
                  if (player.isAdventureMode()) {
                     ...
                     Optional<ImmutableBlockState> optionalCustomState = BlockStateUtils
                           .getOptionalCustomBlockState(againstBlockState);
                     if (optionalCustomState.isEmpty()) {
                        if (!AdventureModeUtils.canPlace(context.getItem(), context.getLevel(), againstPos,
                              againstBlockState)) {
                           return InteractionResult.FAIL;
                        }
                     } else {
                        ...
                        if (!AdventureModeUtils.canPlace(context.getItem(), context.getLevel(), againstPos,
                              Config.simplifyAdventurePlaceCheck() ? customState.vanillaBlockState().literalObject()
                                    : againstBlockState)) {
                           return InteractionResult.FAIL;
                        }
                     }
                  ...
                  CustomBlockAttemptPlaceEvent attemptPlaceEvent = new CustomBlockAttemptPlaceEvent(bukkitPlayer,
                        placeLocation.clone(), blockStateToPlace, DirectionUtils.toBlockFace(context.getClickedFace()),
                        bukkitBlock, context.getHand());
               ...
               if (player != null) {
                  BlockPlaceEvent bukkitPlaceEvent = new BlockPlaceEvent(bukkitBlock, previousState, againstBlock,
                        (ItemStack) context.getItem().getItem(), bukkitPlayer, true,
                  ...
                  CustomBlockPlaceEvent customPlaceEvent = new CustomBlockPlaceEvent(bukkitPlayer,
                        placeLocation.clone(), blockStateToPlace, world.getBlockAt(placeLocation), context.getHand());
               ...
               WorldPosition position = new WorldPosition(context.getLevel(), (double) pos.x() + 0.5,
                     (double) pos.y() + 0.5, (double) pos.z() + 0.5);
   ...
   protected boolean canPlace(BlockPlaceContext context, ImmutableBlockState state) {
      try {
         ...
         boolean defaultReturn = (!this.checkStatePlacement()
               || FastNMS.INSTANCE.method$BlockStateBase$canSurvive(blockState, world, blockPos))
               && (Boolean) CoreReflections.method$ServerLevel$checkEntityCollision.invoke(world, blockState, player,
                     voxelShape, blockPos, true);
         ...
         BlockCanBuildEvent canBuildEvent = new BlockCanBuildEvent(block,
               cePlayer != null ? (org.bukkit.entity.Player) cePlayer.platformPlayer() : null, blockData, defaultReturn,
               context.getHand() == InteractionHand.MAIN_HAND ? EquipmentSlot.HAND : EquipmentSlot.OFF_HAND);
