...
public class CustomBlockData implements PersistentDataContainer {
    ...
    public static CustomBlockData from(Level level, BlockPos pos) {
        ...
        return new CustomBlockData(level.getWorld().getBlockAt(pos.getX(), pos.getY(), pos.getZ()), CraftEnginePolyfills.instance());
    }
    ...
    public <T> void set(TypedKey<T> key, T value) {
        ...
        if (key.isCustom()) {
            ...
            if(customType.isContainerSerializer()){
                PersistentDataContainer container = chunk.getPersistentDataContainer().getAdapterContext().newPersistentDataContainer();
