...
public class ChangeOverTimeBlockBehavior extends BukkitBlockBehavior {
  ...
  public void randomTick(Object thisBlock, Object[] args, Callable<Object> superMethod) throws Exception {
    ...
    if (optionalCurrentState.isEmpty())
      return; 
    ImmutableBlockState newState = ((CustomBlock)optionalNewCustomBlock.get()).getBlockState(((ImmutableBlockState)optionalCurrentState.get()).propertiesNbt());
    BukkitExistingBlock blockInWorld = (BukkitExistingBlock)level.getBlockAt(LocationUtils.fromBlockPos(blockPos));
    BlockFormEvent event = new BlockFormEvent(blockInWorld.block(), BlockStateUtils.fromBlockData(newState.customBlockState().literalObject()).createBlockState());
    if (event.callEvent()) {
      FastNMS.INSTANCE.method$LevelWriter$setBlock(level.serverWorld(), blockPos, newState.customBlockState().literalObject(), UpdateOption.UPDATE_ALL_IMMEDIATE.flags());
  ...
  private boolean shouldChange(Object[] args) {
    return (RandomUtils.generateRandomFloat(0.0F, 1.0F) < this.delay);
  }
