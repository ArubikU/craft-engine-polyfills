...
public class BubbleBlockBehavior extends BukkitBlockBehavior {
  ...
  public BubbleBlockBehavior(CustomBlock customBlock, boolean drag, boolean direction, int limit) {
    ...
    this.limit = limit;
    if (drag) {
      this.blockState = LazyReference.lazyReference(() -> CraftEngine.instance().blockManager().createBlockState("minecraft:bubble_column[drag=true]"));
    } else {
      this.blockState = LazyReference.lazyReference(() -> CraftEngine.instance().blockManager().createBlockState("minecraft:bubble_column[drag=false]"));
    } 
  }
  ...
  public void tick(Object thisBlock, Object[] args, Callable<Object> superMethod) throws Exception {
    ...
    if (this.direction) {
      ...
      for (int i = 0; i < this.limit; ) {
        ...
        if (ft == MFluids.WATER) {
          BukkitExistingBlock blockInWorld = (BukkitExistingBlock)bukkitWorld.getBlockAt(current);
          blockInWorld.block().setBlockData(BlockStateUtils.fromBlockData(((BlockStateWrapper)this.blockState.get()).literalObject()));
          i++;
        } 
      } 
    } else {
  ...
  private void updateNeighbours(Object arg1, Object arg2, Object thisBlock) {
    ...
    if (this.direction) {
      ...
      for (int i = 0; i < this.limit; i++) {
        current = current.above();
        if (FastNMS.INSTANCE.method$BlockGetter$getBlockState(level, LocationUtils.toBlockPos(current)) != MBlocks.BUBBLE_COLUMN)
          break; 
        BukkitExistingBlock blockInWorld = (BukkitExistingBlock)bukkitWorld.getBlockAt(current);
