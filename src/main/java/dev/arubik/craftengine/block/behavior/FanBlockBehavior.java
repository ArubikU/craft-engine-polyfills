...
public class FanBlockBehavior extends BukkitBlockBehavior {
  ...
  private static final Set<String> REDSTONE_SOURCE_IDS = Set.of("minecraft:redstone_block", "minecraft:redstone_torch", "minecraft:wall_redstone_torch");
  
  private static final Map<Direction, Vector> DIRECTION_VECTORS = Map.of(Direction.NORTH, new Vector(0, 0, -1), Direction.SOUTH, new Vector(0, 0, 1), Direction.WEST, new Vector(-1, 0, 0), Direction.EAST, new Vector(1, 0, 0), Direction.UP, new Vector(0, 1, 0), Direction.DOWN, new Vector(0, -1, 0));
  ...
  public FanBlockBehavior(CustomBlock customBlock, Property<Direction> facing, Property<Boolean> powered, Property<Integer> powerLevel, int tickDelay, Particle particle, int maxPushDistance, Set<Key> passableBlocks, boolean redstoneAffectsPush) {
    super(customBlock);
  ...
  public void tick(Object thisBlock, Object[] args, Callable<Object> superMethod) {
    ...
    if (blockState == null || blockState.isEmpty() || !((Boolean)blockState.get(this.poweredProperty)).booleanValue())
      return; 
  ...
  private void applyPushAndParticles(BukkitWorld world, BlockPos targetPos, Direction facing, int pushStrength) {
    ...
    Vector pushVector = ((Vector)DIRECTION_VECTORS.getOrDefault(facing, new Vector(0, 0, 0))).clone().multiply(pushStrength * 0.1D);
    double jitter = 0.2D;
  ...
  private void updateActivationFromNearbyRedstone(Object stateObj, Object level, Object posObj) {
    ImmutableBlockState blockState = BukkitBlockManager.instance().getImmutableBlockState(BlockStateUtils.blockStateToId(stateObj));
    ...
    if (needsUpdate(blockState, shouldPower, targetPowerLevel)) {
      ...
      FastNMS.INSTANCE.method$LevelWriter$setBlock(level, posObj, newState.customBlockState().literalObject(), UpdateOption.UPDATE_ALL.flags());
    } 
  }
  ...
  private boolean needsUpdate(ImmutableBlockState state, boolean shouldPower, int targetPowerLevel) {
    if (((Boolean)state.get(this.poweredProperty)).booleanValue() != shouldPower)
      return true; 
    if (this.powerLevelProperty != null && !Objects.equals(state.get(this.powerLevelProperty), Integer.valueOf(targetPowerLevel)))
      return true; 
    return false;
  }
  ...
  private RedstoneInfo analyzeRedstone(Object level, Object posObj) {
    ...
    for (Direction dir : Direction.values()) {
      BukkitExistingBlock neighbor = (BukkitExistingBlock)world.getBlockAt(pos.relative(dir));
      if (neighbor != null) {
  ...
  public static class Factory implements BlockBehaviorFactory {
    public BlockBehavior create(CustomBlock block, Map<String, Object> arguments) {
      ...
      List<String> passableBlocksList = (List<String>) arguments.getOrDefault("passableBlocks", List.of("minecraft:air"));
      ...
      boolean redstoneAffectsPush = Boolean.parseBoolean(arguments.getOrDefault("redstoneAffectsPush", Boolean.valueOf(true)).toString());
      return (BlockBehavior)new FanBlockBehavior(block, facing, powered, powerLevel, tickDelay, particle, maxPushDistance, passableBlocks, redstoneAffectsPush);
    }
  }
}
