...
public final class ConveyorMath {
    ...
    public static Vector3f startPoint(int stepX, int stepZ, int slopeStepY) {
        return startPoint(stepX, stepZ, slopeStepY, BELT_TOP_Y);
    }

    /** Same as {@link #startPoint(int, int, int)} but at a caller-chosen carry height instead of
     *  the hardcoded {@link #BELT_TOP_Y} — see {@link BeltType#height()}. */
    public static Vector3f startPoint(int stepX, int stepZ, int slopeStepY, float height) {
        float yOff = slopeStepY > 0 ? -0.5f : (slopeStepY < 0 ? 0.5f : 0.0f);
        return new Vector3f(0.5f - (float)stepX * 0.5f, height + yOff + ConveyorMath.slopeLift(slopeStepY), 0.5f - (float)stepZ * 0.5f);
    }
    ...
    public static Vector3f endPoint(int stepX, int stepZ, int slopeStepY) {
        return endPoint(stepX, stepZ, slopeStepY, BELT_TOP_Y);
    }

    /** Same as {@link #endPoint(int, int, int)} but at a caller-chosen carry height instead of the
     *  hardcoded {@link #BELT_TOP_Y} — see {@link BeltType#height()}. */
    public static Vector3f endPoint(int stepX, int stepZ, int slopeStepY, float height) {
        float yOff = slopeStepY > 0 ? 0.5f : (slopeStepY < 0 ? -0.5f : 0.0f);
        return new Vector3f(0.5f + (float)stepX * 0.5f, height + yOff + ConveyorMath.slopeLift(slopeStepY), 0.5f + (float)stepZ * 0.5f);
    }
