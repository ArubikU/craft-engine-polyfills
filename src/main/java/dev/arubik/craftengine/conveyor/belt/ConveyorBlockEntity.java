...
ConveyorReceiver {
    ...
    // ---- Fixed tick-scheduling constants (not belt_types-configurable) ----
    ...
    // ---- Belt-line tunables, set at construction from BeltType (see ConveyorBehavior.Factory) ----
    ...
    /** This block's own {@link BeltType}, resolved lazily by block id and cached; {@code null} if
     *  no matching {@code belt_types} entry. */
    private BeltType cachedBeltType;
    private boolean beltTypeResolved = false;

    private BeltType beltType() {
        if (!this.beltTypeResolved) {
            this.beltTypeResolved = true;
            try {
                BlockDefinition def = (BlockDefinition) this.blockEntity().blockState().owner().value();
                this.cachedBeltType = BeltType.byBlockId(def.id());
            } catch (Throwable ignored) {
                this.cachedBeltType = null;
            }
        }
        return this.cachedBeltType;
    }

    // Belt-type-derived values delegate to BeltRuntime.

    public float carryHeight() {
        return BeltRuntime.carryHeight(this.beltType());
    }

    public float itemScale() {
        return BeltRuntime.itemScale(this.beltType());
    }

    private double pickupRadius() {
        return BeltRuntime.pickupRadius(this.beltType());
    }

    private int maxLength() {
        return BeltRuntime.maxLength(this.beltType());
    }

    public float currentProgressPerTick() {
        return BeltRuntime.progressPerTick(this.beltType(), this.effectiveRpm, this.baseRpm, this.baseTravelTicks);
    }

    private boolean hasRequiredPower(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos) {
        Level level;
        try {
            level = (Level) world.world().minecraftWorld();
        } catch (Throwable ignored) {
            return true;
        }
        BlockPos nmsPos = new BlockPos(pos.x(), pos.y(), pos.z());
        return BeltRuntime.hasRequiredPower(this.beltType(), level, nmsPos);
    }

    public static List<Player> viewersOf(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos) {
    ...
    public static ContraptionBoundary contraptionOf(CEWorld world) {
    ...
    /** How far along its OWN travel the front (furthest-advanced) carried item is, 0..1 (1.0 =
     *  fully arrived at this segment's exit, blocked from handing off further) — -1 if nothing is
     *  carried. {@code peekCarried()}/{@code has_item} answer "is anything here at all", which is
     *  true for an item that just entered and is still mid-transit; a caller that wants only items
     *  actually STALLED at the exit (e.g. a machine pulling from a side-adjacent feeding belt, not
     *  one it sits directly on) needs this instead. */
    public float frontProgress() {
        int i = this.frontSlot();
        return i < 0 ? -1.0f : this.progress[i];
    }

    // ---- Public per-slot API (BeltItem, script side) — a segment can carry several items at
    // once (see `slots`), each independently in transit; the front-slot-only take/put/peek/progress
    // above are a convenience for the common "just the one nearest the exit" case, these let a
    // caller enumerate/target ANY slot directly. ----------------------------------------------
    ...
    public boolean isSlotEmpty(int i) {
        return i < 0 || i >= this.slots || this.slotEmpty(i);
    }

    /** 0..1, -1 if slot index is out of range or empty. */
    public float slotProgress(int i) {
        return this.isSlotEmpty(i) ? -1.0f : this.progress[i];
    }

    public ItemStack getSlotItem(int i) {
        return this.isSlotEmpty(i) ? null : this.bukkitSlot(i);
    }

    /** Removes and returns whatever is in slot {@code i} specifically (unlike {@link #takeSlot()},
     *  which always targets the front/furthest-advanced slot) — null if out of range or empty. */
    public ItemStack takeSlotAt(int i) {
    ...
    /** The random per-item Y-rotation jitter (radians) this slot's display was given when it
     *  entered — purely cosmetic (visual variety so a line of identical items doesn't look
     *  perfectly uniform), 0 if the slot is empty/out of range. */
    public float slotJitter(int i) {
        return this.isSlotEmpty(i) ? 0.0f : this.jitter[i];
    }

    /** The world direction this slot's item entered FROM (opposite of the neighbor that handed it
     *  off), or null if empty/out of range/not recorded — the entry side, along with this
     *  segment's own facing, is what its ConveyorItemDisplay orients the item model against. */
    public Direction slotEntryDir(int i) {
        return this.isSlotEmpty(i) ? null : this.entryDir[i];
    }
    ...
    private void serverTick(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos) {
        ...
        if (this.effectiveRpm > 0.0f && !this.hasRequiredPower(world, pos)) {
            this.effectiveRpm = 0.0f;
        }
        this.maybeUpdateActivated(world, pos, this.effectiveRpm > 0.0f);
        if (++this.tickCounter >= PICKUP_INTERVAL) {
            this.tickCounter = 0;
            if (this.backHasRoom()) {
        ...
        float inc = this.currentProgressPerTick();
        if (inc > 0.0f) {
            float gap = this.spacing();
    ...
    private void tryPickup(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos) {
        try {
            ...
            double r = this.pickupRadius();
            AABB aabb = new AABB(cx - r, cy - r, cz - r, cx + r, cy + r, cz + r);
            Item nearest = null;
    ...
    private boolean tryHandOff(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos, Direction facing, int i, float startProgress) {
        ...
        if (this.tryConnectsToContainer(world, pos, facing, item)) {
            this.despawnSlot(world, pos, i);
            this.clearSlot(i);
            return true;
        }
    ...
    /** Fallback when {@link #receiverAt} found no {@link ConveyorReceiver} — see {@link
     *  BeltRuntime#tryPushIntoConnectedContainer}. */
    private boolean tryConnectsToContainer(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos,
        ...
        return BeltRuntime.tryPushIntoConnectedContainer(this.beltType(), level,
                ConveyorBlockEntity.exitCandidates(pos, facing), item);
    }
    ...
    private boolean redstonePoweredThrottled(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos) {
        if (this.redstoneScanCooldown < 0) {
            this.redstoneScanCooldown = Math.floorMod(pos.x() * 31 + pos.y() * 17 + pos.z(), REDSTONE_SCAN_INTERVAL);
            this.redstonePoweredCache = this.redstonePowered(world, pos);
        } else if (--this.redstoneScanCooldown <= 0) {
            this.redstoneScanCooldown = REDSTONE_SCAN_INTERVAL;
            this.redstonePoweredCache = this.redstonePowered(world, pos);
        }
    ...
    private Vector3f startRel(Direction facing) {
        return ConveyorMath.startPoint(facing.stepX(), facing.stepZ(), this.slope().stepY(), this.carryHeight());
    }

    private Vector3f endRel(Direction facing) {
        return ConveyorMath.endPoint(facing.stepX(), facing.stepZ(), this.slope().stepY(), this.carryHeight());
    }
    ...
    private void renderAll(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos, Direction facing) {
        ...
        for (int i = 0; i < this.slots; ++i) {
            ...
            if (p < 0.5f) {
                Vector3f entryPt = inLine ? this.startRel(facing) : new Vector3f(0.5f + (float)entry.stepX() * 0.5f, this.carryHeight(), 0.5f + (float)entry.stepZ() * 0.5f);
    ...
    public boolean extend(CEWorld world, net.momirealms.craftengine.core.world.BlockPos pos, Direction facing, ConveyorSlope slope) {
        ...
        if (this.lengthFromHere(world, facing) >= this.maxLength()) {
            return false;
        }
        try {
