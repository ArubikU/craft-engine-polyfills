...
/**
 * Pins the rule that decides which {@link BearingType} a contraption is written to disk as.
 *
 ...
 * back out of the world, defaulting to {@link BearingType#ROTATIONAL} when nothing was found. That is
 * correct for LINEAR/ROTATIONAL, whose bearing block stays put and pins the structure to it.
 *
 * <p>It is wrong for {@link BearingType#PHYS}, and it fails silently. A phys contraption has no pinning
...
class BearingTypePersistenceTest {
    ...
    private static ContraptionState state(BearingType type) {
    ...
    void physSurvivesAnAnchorThatIsNoLongerThere() {
        ...
        assertEquals(BearingType.PHYS,
                BlockAnchoredContraptionStore.typeToPersist(state(BearingType.PHYS), null, ANCHOR));
    }

    @Test
    @DisplayName("the recorded type wins over the world for every kind, not just PHYS")
    void theRecordedTypeIsAuthoritative() {
        assertEquals(BearingType.LINEAR,
                BlockAnchoredContraptionStore.typeToPersist(state(BearingType.LINEAR), null, ANCHOR));
        assertEquals(BearingType.ROTATIONAL,
                BlockAnchoredContraptionStore.typeToPersist(state(BearingType.ROTATIONAL), null, ANCHOR));
        assertEquals(BearingType.MINECART,
                BlockAnchoredContraptionStore.typeToPersist(state(BearingType.MINECART), null, ANCHOR));
    }
    ...
    void fallsBackWhenNothingKnowsTheType() {
        ...
        assertEquals(BearingType.ROTATIONAL, BlockAnchoredContraptionStore.typeToPersist(state(null), null, ANCHOR));
        assertEquals(BearingType.ROTATIONAL, BlockAnchoredContraptionStore.typeToPersist(null, null, ANCHOR));
    }
}
