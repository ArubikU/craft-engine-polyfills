...
class RpmPropagationTest {
    ...
    /** Speed a gearbox hands out through {@code out}, when driven from {@code in}. */
    private static float gearbox(float rpm, Face in, Face out) {
        return rpm * RpmPropagation.gearboxModifier(out.axis, out.sign, in.axis, in.sign);
    }

    @Test
    @DisplayName("the shaft OPPOSITE the driven face is reversed, not passed straight through")
    void gearboxReversesTheCollinearFace() {
        // This is the rule I originally got backwards. A gearbox is a pair of meshed bevel gears,
        // so the far end of the same shaft line turns the other way.
        for (Face in : Face.values()) {
            assertEquals(-32f, gearbox(32f, in, in.opposite()), 1e-6,
                    "driven from " + in + ", the " + in.opposite() + " face must reverse");
        }
    }

    @Test
    @DisplayName("the driven face itself reports unchanged (propagating back to the source)")
    void gearboxSourceFaceIsIdentity() {
        for (Face in : Face.values()) {
            assertEquals(32f, gearbox(32f, in, in), 1e-6);
        }
    }

    @Test
    @DisplayName("a perpendicular face follows the axis-direction sign, not a fixed list")
    void gearboxPerpendicularFollowsAxisDirection() {
        // Create: direction.getAxisDirection() == source.getAxisDirection() ? -1 : 1
        assertEquals(-32f, gearbox(32f, Face.NORTH, Face.WEST), 1e-6, "both negative -> reversed");
        assertEquals(-32f, gearbox(32f, Face.SOUTH, Face.EAST), 1e-6, "both positive -> reversed");
        assertEquals(32f, gearbox(32f, Face.NORTH, Face.EAST), 1e-6, "opposite signs -> kept");
        assertEquals(32f, gearbox(32f, Face.SOUTH, Face.WEST), 1e-6, "opposite signs -> kept");
    }

    @Test
    @DisplayName("the whole 6x6 face table matches Create's getAxisModifier")
    void gearboxTableMatchesCreate() {
        for (Face in : Face.values()) {
            for (Face out : Face.values()) {
                // Reference implementation, transcribed from Create.
                float expected = (out.axis == in.axis)
                        ? (out == in ? 1f : -1f)
                        : (out.sign == in.sign ? -1f : 1f);
                assertEquals(expected, RpmPropagation.gearboxModifier(out.axis, out.sign, in.axis, in.sign),
                        1e-6, "in=" + in + " out=" + out);
            }
        }
    }
    ...
    @Test
    @DisplayName("the split moves with the input face — the old fixed compass list could not")
    void gearboxSplitFollowsTheDrivenFace() {
        // gearbox_h hardcoded north/south = same, east/west = inverted. Driving from EAST then
        // gave the wrong sign on every face; here the answer tracks the actual input.
        assertEquals(-32f, gearbox(32f, Face.NORTH, Face.SOUTH), 1e-6);
        assertEquals(-32f, gearbox(32f, Face.EAST, Face.WEST), 1e-6);
        // ...and the same output face flips meaning depending on where power came in.
        assertEquals(-32f, gearbox(32f, Face.NORTH, Face.WEST), 1e-6);
        assertEquals(32f, gearbox(32f, Face.SOUTH, Face.WEST), 1e-6);
    }

    @Test
    @DisplayName("gearbox is consistent for the vertical axis too")
    void gearboxVertical() {
        assertEquals(-32f, gearbox(32f, Face.UP, Face.DOWN), 1e-6, "collinear reverses");
        assertEquals(-32f, gearbox(32f, Face.UP, Face.EAST), 1e-6, "both positive -> reversed");
        assertEquals(32f, gearbox(32f, Face.UP, Face.WEST), 1e-6, "opposite signs -> kept");
    }

    @Test
    @DisplayName("an undriven gearbox outputs nothing rather than a phantom direction")
    void undrivenGearboxOutputsZero() {
        assertEquals(0f, gearbox(0f, Face.NORTH, Face.EAST), 1e-6);
    }

    @Test
    @DisplayName("a gearbox driving a shaft does not then destroy itself")
    void gearboxDoesNotSelfDestruct() {
        // The crash: the gearbox drives a shaft that therefore carries a reversed value. If the
        // gearbox could pull from it, +32 against -32 would trip the conflict rule and destroy a
        // block. The distance rule must keep it from ever reading its own output.
        int gearboxDist = 1;
        float shaftRpm = gearbox(32f, Face.NORTH, Face.EAST);
        assertEquals(32f, shaftRpm, 1e-6);

        int shaftAsProvider = RpmPropagation.providerDistance(true, true, gearboxDist + 1);
        assertEquals(2, shaftAsProvider, "an emitting relay keeps its distance");
        assertFalse(RpmPropagation.canPullFrom(shaftAsProvider, gearboxDist),
                "the gearbox must refuse to pull from the shaft it drives");
        assertFalse(RpmPropagation.isConflict(32f, 0f), "an unpowered side must never destroy a build");
    }

    // ------------------------------------------------- orphaned kinetic networks

    @Test
    @DisplayName("two meshed cogs stop once the motor is taken away")
    void meshedCogsStopWhenSourceIsRemoved() {
        Node motor = source("motor", 32);
        Node cogA = relay("cogA");
        Node cogB = relay("cogB");
        pullEdge(motor, cogA);
        ...
        for (int i = 0; i < 20; i++) tick(all);
        assertEquals(-32f, cogB.rpm, 1e-6, "running while the motor is there");

        // Break the wheel off: the motor is gone entirely, not just stopped.
        cogA.pullFrom.clear();
        for (int i = 0; i < RpmPropagation.SOURCE_MAX_AGE + 10; i++) tick(all);

        assertEquals(0f, cogA.rpm, 1e-6, "cogA has nothing upstream any more");
        assertEquals(0f, cogB.rpm, 1e-6,
                "cogB must not keep spinning off cogA's stale output");
        assertFalse(cogA.emitting, "a dead relay must stop claiming to emit");
        assertFalse(cogB.emitting);
    }
